...
import { markdownToNotionBlocks, processMarkdownFrontmatter } from './notionUtils';
import { VaultFile } from '../utils/vaultScanner';
...
export default class NotionSyncService {
    ...
    async syncFilesToNotion(files: VaultFile[], vault: Vault): Promise<void> {
        ...
        if (!isConnected) {
            throw new Error('Falha na conexão com a API do Notion. Verifique seu token.');
        }
            ...
            try {
                ...
                if (parts.length === 1) {
                    ...
                    if (parentId) {
                        ...
                        console.warn(`Pasta pai não encontrada para ${folderPath}, usando raiz`);
                        const notionPageId = await this.createFolderPage(
                            this.rootPageId,
                            folderName
                        );
     ...
     */
    private async createFolderPage(parentId: string, folderName: string): Promise<string> {
        try {
